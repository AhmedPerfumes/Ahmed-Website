...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    const hasBogo = cartProducts.some((item) => promotionsContext.some((promo) => promo.buy_products.some((buyItem) => buyItem.product_id === item.product_id)));
    if (!hasBogo) {
      const cleanedCart = cartProducts.map(({ is_coupon, value, ...rest }) => rest);
            ...
            <div className="checkout-form">
              ...
              <div className="checkout__totals-wrapper">
                <div className="sticky-content">
                  ...
                  {!couponData ? ( <input className="coupon-action-btn" type="button" value="APPLY COUPON" onClick={applyCoupon} /> ) : ( <input className="coupon-action-btn remove" type="button" value="REMOVE COUPON" onClick={removeCoupon} /> )}
                  <br /> <br />
                  ...
                  <div className="checkout__payment-methods">
                    ...
                    <div className="form-check">
                      ...
                      <label className="form-check-label" htmlFor="checkout_payment_method_5" style={{display: "inline-flex"}} > Tamara - No interest, No fees.  <TamaraWidget inlineType='4' inlineVariant='text'/> </label>
                    </div>
                  ...
                  <button className="btn btn-primary w-100 text-uppercase" type="submit" disabled={disablePlaceOrder} > {isLoading ? "Loading..." : "Place Order"} </button>
                </div>
              </div>
            </div>
          </form>
