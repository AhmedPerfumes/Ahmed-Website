...
export default function Header14() {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            if (currentScrollY <= 50) {
                ...
                if (currentScrollY - lastShowY > hideThreshold) setScrollState("hidden");
            } else if (currentScrollY < lastScrollY.current) {
                setScrollState("visible");
                lastShowY = currentScrollY;
            }
    ...
    useEffect(() => { if (isPopupOpen && inputRef.current) inputRef.current.focus(); }, [isPopupOpen]);

    // --- Search handler ---
    const handleChange = (e) => setSearchKeyWord(e.target.value);
    const onSearch = (event) => {
        event.preventDefault();
        const cleanedKeyword = removeSpecialCharactersAndAmp(searchKeyWord).split(" ").join("-");
    ...
    const swiperOptions = { autoplay: { delay: 5000 }, modules: [Autoplay, Navigation, EffectFade], pagination: false, slidesPerView: 1, effect: "fade", loop: true };
