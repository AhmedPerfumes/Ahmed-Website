...
export default function AccountOrders() {
  ...
  // Table columns
  const columns = [
    columnHelper.accessor("code", { header: "Order #", cell: info => info.getValue(), enableSorting: true }),
    columnHelper.accessor("created_at", { header: "Date", cell: info => new Date(info.getValue()).toLocaleDateString(), enableSorting: true }),
          ...
          <span>
            <Badge bg={
              val === "processing" ? "info"
                ...
                : val === "returned" ? "warning"
                : val === "cancelled" ? "danger"
                : "secondary"
            } className="rounded-pill px-3 py-1" style={{ fontSize: "0.90em" }}>
              {label}
            </Badge>
          </span>
        );
    ...
    columnHelper.accessor("payment_channel", {
      ...
      cell: info => ({ cod: "Cash on Delivery", paytabs: "PayTabs" }[info.getValue()] || info.getValue()),
      enableSorting: false
    }),
    columnHelper.display({
      id: "actions", header: "Actions",
      cell: ({ row }) => <Button className="btn-rounded btn-link_lg text-uppercase fw-medium" size="sm" style={{ borderRadius: "30px" }} onClick={() => handleView(row.original)}>ORDER DETAILS</Button>,
      enableSorting: false
    }),
  ];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        await Promise.all(json.data.map(async (order) => {
          try {
  ...
  const sections = [
    { key: "processing-shipped", title: "Processing & Shipped", filter: r => ["processing", "shipped"].includes(r.original.status.value) },
    { key: "completed", title: "Completed", filter: r => r.original.status.value === "completed" },
    { key: "returned", title: "Returned", filter: r => r.original.status.value === "returned" },
    { key: "cancelled", title: "Cancelled", filter: r => r.original.status.value === "cancelled" },
  ];
      ...
      <div className="modern-table-responsive">
          ...
          <thead>
            <tr>
              {table.getHeaderGroups()[0].headers.map(header => (
                <th key={header.id} className="text-secondary text-white small" style={{ fontWeight: "600", background: "#000000ff", textTransform: "uppercase", letterSpacing: 1 }}>
                ...
                <tr className="product-summary-row">
                  <td colSpan={columns.length} style={{ padding: 0, background: "#f8fbff" }}>
                    <div style={{
                      ...
                      {(orderSummaries[row.original.id] || []).slice(0, 2).map((prod, i) => (
                        <div key={prod.id} style={{ display: "flex", alignItems: "center", marginRight: 24, minWidth: 0 }}>
                          <img
                            src={prod.product_image ? `${IMG_BASE}storage/${prod.product_image}` : "/no-img.png"}
                            alt={prod.product_name}
                            style={{
                              width: 42, height: 42,
                              objectFit: "cover", borderRadius: 8, marginRight: 10, border: "1.5px solid #e6e6e6"
                            }}
                          />
                          {/* <img
                          ...
                          /> */}
                          <span style={{ fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontSize: "1em" }}>
                            {prod.product_name}
                          </span>
                          <span style={{ color: "#4c4c4c", fontWeight: 400, marginLeft: 10, fontSize: ".97em" }}>
                            {prod.qty} × {Number(prod.price).toFixed(2)}
                          </span>
                        </div>
                      ))}
                      {(orderSummaries[row.original.id] || []).length > 2 &&
                        <span style={{ fontSize: ".95em", fontWeight: 500, color: "#555" }}>
                          +{orderSummaries[row.original.id].length - 2} more
                        </span>
                      }
                      {(orderSummaries[row.original.id] || []).length === 0 &&
                        <span className="text-muted" style={{ fontSize: ".97em" }}>No products</span>
                      }
                    </div>
                  </td>
                </tr>
        ...
        <style jsx>{`
          ...
          @media (max-width: 991px) {
            .modern-table th, .modern-table td { padding: 14px 6px; }
          }
          @media (max-width: 600px) {
            .modern-table thead { display: none; }
            .modern-table tr, .modern-table td { display: block; width: 100%; }
            .modern-table-row {
              border-radius: 14px;
              margin-bottom: 12px;
  ...
  const renderTables = () => sections.map(sec => {
    ...
    <>
      ...
      <style jsx global>{`
        ...
        .nav-tabs .nav-link.active:focus {
          ...
          box-shadow: 0 2px 12px 0 rgba(30,34,55,.10);
        }
      ...
      `}</style>
      <div className="col-lg-9" style={{ fontFamily: "SofiaProRegular, sans‑serif", border: "1px solid #e9e9e9", borderRadius: ".75rem", padding: "50px" }}>
        {/* STATUS TABS */}
        <Nav
          variant="tabs"
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
        ...
        <Modal.Body className="p-4" style={{ minHeight: 350 }}>
          {modalLoading ? (
          ...
          ) : modalDetails && modalDetails.order_products && modalDetails.order_address ? (
            <div className="row g-5 flex-md-row flex-column" style={{ fontFamily: "Inter, Arial, sans-serif" }}>
              {/* LEFT: Order Info */}
              <div className="col-md-6 mb-4 mb-md-0">
                <div className="mb-4">
                  <div className="mb-1 text-uppercase small fw-semibold text-muted">Contact Information</div>
                  <div style={{ fontWeight: 500, fontSize: 18 }}>{modalDetails.order_address[0]?.name}</div>
                  <div>{modalDetails.order_address[0]?.phone}</div>
                  <div>{modalDetails.order_address[0]?.email}</div>
                </div>
                <div className="mb-4">
                  <div className="mb-1 text-uppercase small fw-semibold text-muted">Status</div>
                  <div style={{ fontWeight: 500, fontSize: 18 }}>
                    {
                      modalOrderOuter?.status?.label &&
                      <Badge bg={
                        modalOrderOuter.status.value === "processing" ? "info"
                          : modalOrderOuter.status.value === "shipped" ? "primary"
                          : modalOrderOuter.status.value === "completed" ? "success"
                          : modalOrderOuter.status.value === "returned" ? "warning"
                          : modalOrderOuter.status.value === "cancelled" ? "danger"
                          : "secondary"
                      } className="rounded-pill px-3 py-1" style={{ fontSize: "0.98em" }}>
                        {modalOrderOuter.status.label}
                      </Badge>
                    }
                  </div>
                ...
                <div className="mb-4">
                  <div className="mb-1 text-uppercase small fw-semibold text-muted">Payment Method</div>
                  <div style={{ fontWeight: 500, fontSize: 18 }}>
                    {
                      modalOrderOuter?.payment_channel
                        ? ({ cod: "Cash on Delivery", paytabs: "PayTabs" }[modalOrderOuter.payment_channel] || modalOrderOuter.payment_channel)
                        : "—"
                    }
                  </div>
                </div>
                <div className="mb-4">
                  <div className="mb-1 text-uppercase small fw-semibold text-muted">Shipping Address</div>
                  <div>{modalDetails.order_address[0]?.address}, {modalDetails.order_address[0]?.city}, {modalDetails.order_address[0]?.state}</div>
                </div>
                <div className="mb-4">
                  <div className="mb-1 text-uppercase small fw-semibold text-muted">Billing Address</div>
                  <div>{modalDetails.order_address[0]?.address}, {modalDetails.order_address[0]?.city}, {modalDetails.order_address[0]?.state}</div>
                </div>
              </div>
              {/* RIGHT: Items (scrollable) */}
              <div className="col-md-6">
                <h4 className="mb-4" style={{ fontWeight: 600 }}>Items</h4>
                <div
                  style={{
                    maxHeight: 280,
                    overflowY: "auto",
                  ...
                  {modalDetails.order_products.map((item) => (
                    <div key={item.id} className="d-flex align-items-center mb-4 pb-3 border-bottom">
                      <img
                        ...
                        style={{
                          width: 80, height: 80,
                          objectFit: "cover", borderRadius: 12, marginRight: 18
                        }}
                      />
                       {/* <img
                        src='http://localhost/ahmed-admin/public/storage/products/marj-2.jpg'
                      ...
                      <div className="flex-grow-1">
                        <div style={{ fontWeight: 500, fontSize: 16 }}>{item.product_name}</div>
                        {item.is_gift === 1 && (
                          <div className="small text-dark" style={{ fontWeight: 500 }}>(Free Gift)</div>
                        )}
                      </div>
                ...
                <div className="pt-2 pb-1 d-flex justify-content-between">
                ...
                <div className="pb-1 d-flex justify-content-between">
                  ...
                  <span className="fw-semibold text-danger">
                    -{modalDetails.order_products.reduce((sum, it) => sum + Number(it.discount_amount), 0).toFixed(2)}
                  </span>
                </div>
                <div className="border-top pt-3 mt-3 d-flex justify-content-between align-items-center">
                  <div style={{ fontWeight: 600, fontSize: 22 }}>Subtotal</div>
                  <div style={{ fontWeight: 600, fontSize: 22 }}>
                    {
                      modalDetails.order_products
                        .reduce((sum, it) => sum + Number(it.gross_amount), 0)
                        .toFixed(2)
                    }
                  </div>
                </div>
            ...
            <div className="text-muted py-5 text-center">Order details not available.</div>
          )}
        </Modal.Body>
